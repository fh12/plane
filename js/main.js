...
创建子弹类
 */
function bullet(X,Y,sizeX,sizeY,imgClass,soundSrc,type){
    this.bulletX=X;
    this.bulletY=Y;
    this.imagenode=null;
...
function oddbullet(X,Y){
    bullet.call(this,X,Y,15,37,"bullet bullet1",'music/shot.mp3','friend');
}

/*创建敌军子弹*/
function enemybullet1(X,Y){
    bullet.call(this,X,Y,14,14,"bullet e-bullet1",'music/shot.mp3','enemy');
}
...
function ourplan(X,Y,imgClass,soundSrc,trailName,hasfire) {
    plan.call(this,3,X,Y,70,54,0,660,0,"image/boom.gif",imgClass,soundSrc,'defualt',false);
...
function start(){
    ...
    if(mark1%450==0){
        //小飞机 曲线路
        enemys.push(new enemy(4,0,10,50,34,10,860,random(3,5),"image/boom.gif","enemys1 plane",'music/Explode02.ogg','curve'))
    }
    if(mark1%470==0){
        //小飞机 曲线路
        enemys.push(new enemy(4,0,10,50,34,10,860,random(3,5),"image/boom.gif","enemys1 plane",'music/Explode02.ogg','curve'))
    ...
    if(mark==1000){
        //大飞机
        enemys.push(new enemy(20,57,120,150,173,300,1240,1,"image/boom.gif","enemys5 plane",'music/Explode03.ogg','default'));
    }
    if(mark==3500){
        //大飞机
        enemys.push(new enemy(20,57,120,150,173,300,1240,1,"image/boom.gif","enemys5 plane",'music/Explode03.ogg','default'));
...
*/
    if(mark%bulletFrequency==0){
        bullets.push(new oddbullet(parseInt(selfplan.imagenode.style.left) +27,parseInt(selfplan.imagenode.style.top)-12));
    }
        ...
        for(var j=0;j<enemys.length;j++){
            ...
            if(enemys[j].planisdie==false){
                ...
                if(isCollide(bullets,enemys,i,j)){
                        //敌机血量减子弹攻击力
                        enemys[j].planehp=enemys[j].planehp-bullets[i].bulletattach;
